...
![Baseline vs. Adjusted Model Metric Comparison](data_cleaning/outputs/model_comparison_metrics.png){@figure-comp}

Here, the baseline model represents the initial model run only on the volunteering categories and the adjusted model represents the model including propensity scores. As shown in @figure-comp, the R² score is higher for the adjusted model, meaning that it explains more of the outcome variance. The adjusted model also has a lower AIC which means it was able to fit the data better. Finally, the MSE for the adjusted model is slightly lower than the baseline, indicating greater accuracy in predicting outcomes.
