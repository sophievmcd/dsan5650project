...
- volunteered under around or over 100 hours(SG195) - 196, 197 are 200 and 50
...
To drop variables - over 50% NAN
```{python}
merged_final = merged_df
merged_final = merged_final.drop('RB091M', axis=1)
merged_final = merged_final.drop('RB014', axis=1)
merged_final = merged_final.drop('RC106', axis=1)
merged_final = merged_final.drop('RC070', axis=1)
merged_final = merged_final.drop('RG037', axis=1)
merged_final = merged_final.drop('RV402', axis=1)
merged_final = merged_final.drop('RV401', axis=1)
merged_final = merged_final.drop('RV404', axis=1)

```

Rename columns
```{python}
merged_rename = merged_final.rename(columns={
    'RC036': 'heart_condition',
    'RC271': 'ever_had_depression',
    'RC272': 'alzheimers',
    'RJ005M1': 'employment_status'
})
...
merged_final.to_csv("../clean_data/covariates_2020_final.csv", index=False)
